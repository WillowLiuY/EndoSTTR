...
from model.attention import MultiheadAttentionRelative
...
class Transformer(nn.Module):
    ...
    def _alternating_attn_dense(self, feat, feat_s, pos_enc, pos_idx, pos_enc_s, pos_idx_s, pos_enc_y, pos_idx_y, pos_enc_y_s, pos_idx_y_s, hn: int, visualize: bool = False):
        global layer_idx
        for idx, (self_attn, cross_attn) in enumerate(zip(self.self_attn_layers, self.cross_attn_layers)):
            layer_idx = idx
            ...
            if visualize:
                torch.save(feat, f'feat_self_attn_input_{layer_idx}.dat')

            feat, feat_s = checkpoint(create_custom_self_attn(self_attn), feat, feat_s, 
                                    pos_enc, pos_idx, pos_enc_s, pos_idx_s, 
                                    pos_enc_y, pos_idx_y, pos_enc_y_s, pos_idx_y_s)
            ...
            if visualize:
                torch.save(feat, f'feat_cross_attn_input_{layer_idx}.dat')

            feat, attn_weight = checkpoint(create_custom_cross_attn(cross_attn), feat[:, :hn], feat[:, hn:], 
                                        feat_s[:, :hn], feat_s[:, hn:], pos_enc, pos_idx, 
                                        pos_enc_s, pos_idx_s)
