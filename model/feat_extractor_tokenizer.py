...
class TransitionUp(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, scale: int = 2):
        ...
        if scale == 2:
            ...
            raise ValueError("Unsupported scale value. Only scale=2 or scale=4 are supported.")
...
class Tokenizer(nn.Module):
    ...
    :param block_config: config of dense blocks, number of layers in each block.
    :param backbone_feat_channel: used for skip connections.
    :param hidden_dim: dim of the hidden layer.
    :param growth_rate: num of feature maps added by each layer in a dense block.
    """
    ...
    def forward(self, features: list):
        ...
        if self.dense:
            output_dense = F.avg_pool2d(output, [1,2], stride[1,2])
            return output_dense
        return output
